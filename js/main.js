...
$(function () {
    ...
    //Рівень зайнятості
    var emplHandle = $("#employment-handle");
    window.emplSlider = $("#employment").slider({
        create: function () {
            emplHandle.text($(this).slider("value"));
        },
        slide: function (event, ui) {
            emplHandle.text(ui.value);
        },
        change: calculate,
        min: 0.5,
        max: 0.9,
    ...
    // Середня зарплата
    var salaryHandle = $("#salary-handle");
    window.salarySlider = $("#salary").slider({
        create: function () {
            salaryHandle.text($(this).slider("value"));
        },
        slide: function (event, ui) {
            salaryHandle.text(ui.value);
        },
        change: calculate,
        min: 4000,
        max: 15000,
...
//          "age": ["працездатні", "пенсіонерів"],
        ...
        var employmentVal = emplSlider.slider('value');
        var payersVal = payersSlider.slider('value');
        var pensionVal = pensionSlider.slider('value');
        var esvVal = esvSlider.slider('value');
        var salaryVal = salarySlider.slider('value');

        var workForceCalcVal = obj[ageVal][0] / 1000; // ЧИСЛО працездатних
        $("#result1")[0].innerHTML = "Працездатні: " + workForceCalcVal + " млн осіб";


        var employedCalcVal = Math.round(workForceCalcVal * employmentVal * 1000) / 1000; // ЧИСЛО зайнятих
        $("#result2")[0].innerHTML = "Зайняті: " + employedCalcVal + " млн осіб";


        var payCalcVal = Math.round(employedCalcVal * payersVal * 1000) / 1000; // ЧИСЛО платників
        ...
        // Надходження (платників * ставка внеску * зарплата * 12) = (payCalcVal * esvVal * salaryVal * 12)
        var inVal = Math.round(payCalcVal * esvVal * salaryVal * 12) / 1000; // надходження
